...
% N=6000; 
% P=11;%%%%%%%%%%%%%%%%%%%%%%%%% Add zero-order coefficient
% gamma=rand;
% conf=randn(3)>0;
...
% small_control   = 39;
% small_control   = 9;
small_control   = params.small_control;

while ~done
    
    arSig=zeros(M,M*P);
    for k=2:P %%%%%%%%%%%% Zero-order =0
...
%     fprintf('try time:%i, lambdaMax:%f, lambdaMax_Noise:%f.\n',try_time, lambdaMax, lambdaMaxNoise);
    ...
    if lambdaMax<.93 && lambdaMaxNoise<.93;
...
for i=2:N;
    ...
    for j=1:nOrderLoc;
        y(:,i)=y(:,i)+arReshape(:,:,j)*y(:,i-j+1);%%%%%%%%%%%% Zero-order
    end
